...
class HomeNeearByPage extends StatefulWidget {
  final bool isParty;
  const HomeNeearByPage({Key? key, this.isParty = false}) : super(key: key);

  @override
  State<HomeNeearByPage> createState() => _HomeNeearByPageState();
}

class _HomeNeearByPageState extends State<HomeNeearByPage> {
  ...
  void initState() {
    super.initState();
    position = Get.find<VideoController>().position;
    getUsers();
  }
  ...
  getUsers({
    ...
    position ??= await Get.find<VideoController>().getLocation();
    position = Get.find<VideoController>().position;
    if (getMore) {
      var user = await FirebaseFirestore.instance
          .collection('User')
          .startAfterDocument(users.last)
          .where(widget.isParty ? 'isParty' : 'isLive', isEqualTo: true)
          .limit(10)
          .get();
      List newUser = user.docs;
      newUser.removeWhere(
      ...
      for (var element in users) {
        debugPrint(element['long'].toString());
      }
      users.removeWhere(
        (element) =>
            calculateDistance(
  ...
  Widget build(BuildContext context) {
    ...
    if (users.isEmpty) {
      return Center(
        child: Text(
          widget.isParty ? "No Party Found" : "No Live Stream Available",
